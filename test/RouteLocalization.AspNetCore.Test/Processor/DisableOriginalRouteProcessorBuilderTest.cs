	...
	public class DisableOriginalRouteProcessorBuilderTest
	{
		...
		public void BuildWithControllerActionsSelectionBuildsCorrectProcessor2()
		{
			...
			Assert.IsTrue(originalController.Actions.First()
				...
				.ActionConstraints.OfType<NeverAcceptActionContraint>()
				.Any());
			Assert.IsTrue(originalController.Actions.First().Selectors.Last().ActionConstraints.OfType<NeverAcceptActionContraint>().Any());
		}
		...
		public void BuildWithControllerSelectionMixBuildsCorrectProcessor()
		{
			...
			foreach (SelectorModel selectorModel in translatedController3.GetOriginalModel(GetLocalizedRouteMarker()).Selectors.Take(1))
			{
